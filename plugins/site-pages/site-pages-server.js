 ...
 * page module, expanding ecommerce product routes, and guaranteeing a home entry.
 *
 * @param {{ routerEntries: Array<{ routePath: string, componentName: string, routerFileId: string, imports: Record<string, string> }>, titlesByPageFile: Record<string, string>, ecommercePageFiles: Record<string, boolean> }} index
...
export async function buildPageList(index, resolveModule, projectRoot) {
	...
	if (!pages.some((page) => page.path === '/')) {
		console.error('[site-pages] Home route "/" not found by route scan; reporting a fallback entry');
		pages.unshift({ path: '/', label: HOME_ROUTE_LABEL, helmetTitle: null, component: null, file: null });
	}
